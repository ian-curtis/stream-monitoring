...
download_discovery_document <- function(url, path = NULL) {
  # url <- make_discovery_url(id)
  # Google seems to have changed their url structure. I inputted the url myself
  dd <- httr::GET(url)
...
download_discovery_document(url = "https://forms.googleapis.com/$discovery/rest?version=v1")
drive <- "app/forms-v1_2024-03-21.json"
dd <- read_discovery_document(drive)
...
methods <- jsonlite::toJSON(methods)
jsonlite::write_json()
